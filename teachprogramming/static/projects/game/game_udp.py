...
class NetworkUDP():
    ...
    def __init__(self, addr_to='127.0.0.1', port=5005):
        ...
        self.addr_to = (addr_to, port)
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        try:
            self.sock.bind(('0.0.0.0', port))
        except OSError:
            log.debug('cant bind socket - is another client is already running on this port?')
    ...
    def _recv_thread(self) -> None:
        while True:
            self.recv_queue.put_nowait(self.sock.recvfrom(1024))
            #log.debug(f"received bytes: {data} from {self.addr_to}")
            #try:
            #except queue.ShutDown:  # TODO: may be unneeded
            #    log.error('wha?')
            #    break

    def send(self, data: bytes) -> None:
        self.sock.sendto(data, self.addr_to)

    def get(self) -> UDPMessage:
        try               : return self.UDPMessage(*self.recv_queue.get_nowait())
...
class UDPDemo(PygameBase):
    ...
    def loop(self, s, frame):
        while msg := self.network.get():
            (_x, _y) = map(int, msg.data.decode('utf8').split(','))
